...
for b in B:
    ...
    for s in re.split(r'(?<=[.다])\s+',strip(b['content'])):
        s=s.strip()
        if len(s)>=2: sents.append((s,b))          # FIX: 10 -> 2 (면책 리스트 보존)
...
def flush():
    ...
    if cur:
        ...
        if len(txt)>=30:
            chunks.append({"text":txt,"meta":{**cur[-1][1]['meta'],"pages":pages,"btype":"text"}})
        elif chunks and chunks[-1]['meta'].get('btype')=='text':   # FIX: 짧은 조각 앞 청크에 병합
...
flush()
nbiman=sum(1 for c in chunks if '비만' in c['text'] or 'E66' in c['text'])
print("CHUNKS",len(chunks),"표",sum(1 for c in chunks if c['meta']['btype']=='table'),"| 비만/E66 포함 청크:",nbiman)
for c in chunks:
    if '비만' in c['text']: print("  비만청크 p%s:"%c['meta']['pages'], c['text'][:150].replace(chr(10)," ")); break
...
r=hyb("비만으로 입원하면 보상되나 면책",5)
print("비만 검색 top5 페이지:",[p.payload.get('pages') for p in r],"| 비만포함:",any('비만' in p.payload['text'] for p in r))
pg=lambda p:(p.payload.get('pages') or [])
hit={1:0,3:0,5:0}; mrr=0
for g in golden:
    res=hyb(g['question']); ranks=[i+1 for i,p in enumerate(res) if g['gp'] in pg(p)]; f=ranks[0] if ranks else None
    for k in (1,3,5):
        if f and f<=k: hit[k]+=1
    mrr+=(1/f) if f else 0
n=len(golden); print("골든 회귀확인:",{f"hit@{k}":round(hit[k]/n,3) for k in (1,3,5)},"MRR",round(mrr/n,3))
print("FIX_DONE")
