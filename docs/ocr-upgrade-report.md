...
- **결정(정정)**: 사후 재검증 결과 **OCR은 병목이 아님**(남은 실패 2건 모두 생성/추론, OCR엔 내용 정확히 존재) → **Chandra 전면 재-OCR 보류**. 상세는 §10.
...
## 10. 사후 재검증 — OCR은 병목이 아니었음 (결론 정정)
Chandra 채택 후 전면 재-OCR 직전, "OCR이 실제 실패의 원인인가"를 검증한 결과 **아님**으로 판명.
- 최적 표 구성(L2c markdown)에서 낮은-ac 표/수치 질문은 단 **2개**뿐:
  - "통원 3만원 이하 서류 제출?" (ac 0.16): 표 숫자가 아닌 서류규정 텍스트 — 내용은 Unlimited OCR에 정확히 존재.
  - "자기부담 20%=250만원이면 실제 본인부담?" (ac 0.49, faithfulness 0.0): 계산·추론 문제. 연 200만원 상한이 OCR에 정확히 존재("연간 200만원을 초과하는 경우 그 초과금액은 보상"). 실패는 생성이 상한을 적용 못 한 추론 오류.
- 두 실패 모두 OCR 추출 오류가 아니라 **생성/추론 문제**. p21 표도 Unlimited가 이미 정확히 추출(bake에서 Chandra와 동급).
- 표 병목의 실질 원인은 **표현 방식**(HTML/markdown grid)이었고 **L2c(markdown)로 이미 해소**됨.
- **결론: Chandra 전면 재-OCR은 데이터상 정당화되지 않음 → 보류.** 남은 레버는 생성-추론(계산 질문)이며, 이는 더 강한/추론형 생성 모델 또는 명시적 계산 스텝의 문제이지 OCR이 아님.
- 교훈: OCR 후보를 벤치·bake-off로 고르기 전에 **"실패의 원인이 OCR인가"를 먼저 측정**했어야 함. 이 재검증이 5시간 재-OCR을 회피시킴.
