...
| 생성 | **ThinkingCap-Qwen3.6-27B** (thinking, FP8) — 초기 Qwen3-8B 대비 답변정확도 +36% (아래 최종증명) |
| 평가 심판 | Qwen3-32B-AWQ (LLM-as-judge, 별도 인스턴스) |
| 오케스트레이션 | LangGraph |
| 인프라 | DGX Spark GB10 (ARM64/CUDA13). vLLM: 안정 아키텍처 nv26.01, 신규 아키텍처(qwen3_5) nv26.06/vLLM 0.22 |
...
## 표·수치 정밀추출 — 검증 노드 및 32B 재채점
표 표현 방식(HTML/markdown)과 수치 자기검증 노드(L5)를 32B 심판으로 평가했다.

| 구성 | 전체 ac | 표 ac | cr | faith |
|---|---|---|---|---|
| L2b HTML 표 | 0.699 | 0.604 | 0.932 | 0.875 |
| L2c markdown 표 | 0.644 | 0.604 | 0.949 | 0.938 |
| L5 = L2c + 수치검증 노드 | 0.643 | 0.604 | 0.949 | 0.938 |
| L6 = 행 단위 records | 0.622 | 0.439 | 0.948 | 0.900 |

- 8B 심판(표 ac 0.853)과 달리 32B 심판은 표 질문 정확도를 0.604로 평가 — 표·수치 추출 문제가 실재함을 확인.
- 수치 자기검증 노드(생성 후 같은 모델이 수치를 문맥과 대조·교정)는 개선 없음(0.604→0.604). 같은 모델의 체계적 표 오독은 자기검증으로 잡히지 않는다(첫 오독을 재검증에서도 동일하게 반복).
- 행 단위 레코드 변환(표를 `행키: 열=값`로 평탄화)은 표 ac를 오히려 악화(0.604→0.439). 원인: colspan/rowspan 병합셀·다단 헤더를 위치기반 파서가 오정렬하여 확신에 찬 오답을 생성. raw grid는 colspan 정보를 보존해 LLM이 부분 활용하나 레코드화는 이를 소실.
- (당시) 결론: 표 정확도의 병목은 표현 형식이 아니라 표 구조 파싱(병합셀·계층 헤더, 본 코퍼스 TEDS 0.55)으로 추정. 실질 해법 = colspan/rowspan 전개 · 표 영역 VLM 재판독 · 표 특화 추출.
- ★**후속 재검증에서 이 "표 구조 파싱 병목" 가설은 기각됨** (아래 «OCR 병목 재검증» 참조): 실제 잔여 실패는 OCR/파싱이 아니라 **생성-추론**이었고, 27B thinking 생성으로 해소(표 F1 +55%). OCR·파싱 교체는 데이터상 불필요.
...
## OCR 병목 재검증 — OCR은 원인이 아니었음
표·수치 실패를 OCR 품질 탓으로 보고 교체 후보를 조사했다(OmniDocBench·OCRBench v2·CC-OCR 벤치 + 우리 한국어 표 실측 bake-off; `docs/ocr-benchmark.md`, `docs/ocr-upgrade-report.md`). 그러나 **교체 전 "실패의 원인이 OCR인가"를 직접 측정**한 결과 아님으로 판명:
- 최적 표 구성(L2c)에서 낮은-ac 표/수치 질문은 **2건뿐**이었고, 둘 다 OCR이 아닌 생성/추론 실패(자기부담 계산 faithfulness=0, 서류규정 텍스트).
- 필요한 내용(연 200만원 상한·서류규정)은 기존 Unlimited-OCR에 **정확히 존재**. bake-off에서도 우리 표는 Unlimited=Chandra v2 동급.
- 부수 발견: 영/중 벤치 1위 GLM-OCR은 한국어 글자 정확도가 오히려 최악 — **벤치 순위가 한국어로 전이되지 않음**(백본 언어 커버리지가 결정적).
- 결론: 표 병목은 OCR도 표현 형식도 아닌 **생성-추론**. → OCR 교체 보류, 생성 모델 강화로 전환.

## 생성 모델 업그레이드 — 8B → 27B thinking (최종증명)
남은 병목(생성-추론)을 강한 thinking 모델로 해소. `ThinkingCap-Qwen3.6-27B`(arch qwen3_5)는 nv26.01 vLLM(0.13)이 미지원 → **nv26.06/vLLM 0.22.1**로 서빙(GB10에서 네이티브·고속). `docs/generation-upgrade.md`.

동일 컨텍스트(L2c)·동일 32B 심판(F1)으로 8B vs 27B 비교 (순차 배치, 17문항):
| 생성 모델 | 전체 F1 | 표 F1 |
|---|---|---|
| Qwen3-8B | 0.549 | 0.485 |
| **ThinkingCap-27B (thinking)** | **0.744** | **0.752** |
| 델타 | **+0.195 (+36%)** | **+0.267 (+55%)** |

- 이전 실패 질문 대폭 개선: 비만 E66 면책(0.00→0.67), 선택형 공제금액(0.12→0.67), 통원 최고한도(0.50→1.00), 정신과질환 면책(0.67→1.00).
- **표 F1이 가장 크게 개선(+55%)** — 앞선 진단(표 병목=생성-추론)과 일치. 8B의 표 읽기·계산 약점을 thinking 추론이 해소.
- 순차 배치 이유: GB10 단일 GPU에 다중 모델 공존이 불안정 → "한 번에 한 모델만 상주"(생성 배치 → 컨테이너 교체 → 판정 배치)로 우회. 27B가 지속부하에 degrade하여 17/32 커버리지(전량은 안정 환경/단문항 배치로 완주 가능).
