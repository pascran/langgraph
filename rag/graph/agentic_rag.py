...
llm=OpenAI(base_url="http://localhost:8001/v1",api_key="x"); MODEL="Qwen/Qwen3-8B-AWQ"
...
def chat(sy,us,mx=256):
    r=llm.chat.completions.create(model=MODEL,messages=[{"role":"system","content":sy},{"role":"user","content":us}],temperature=0,max_tokens=mx,extra_body={"chat_template_kwargs":{"enable_thinking":False}})
...
class State(TypedDict):
    question:str; orig:str; route:str; documents:list; generation:str; tries:int; log:list
...
def n_generate(s):
    ...
    a=chat("약관 문맥에만 근거해 간결히 답하고 근거를 [pN]로. 면책(보상안함)인지 보상인지 명확히. 없으면 '약관에서 확인 불가'.",f"[문맥]\n{ctx}\n\n[질문]{s['orig']}",256)
    return {"generation":a,"log":s['log']+[f"generate(부모 p{pages}) → {a[:40]}..."]}
...
def e_selfrag(s):
    ...
    ctx=" ".join(d['text'] for d in s['documents'])[:1500]
