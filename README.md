...
- 결론: 표 정확도의 병목은 표현 형식이 아니라 표 구조 파싱(병합셀·계층 헤더, 본 코퍼스 TEDS 0.55)이다. 실질 해법 = colspan/rowspan 전개(grid 정규화) · 표 영역 VLM 재판독 · 표 특화 추출. 표현만 바꾸는 접근(markdown/records)은 파싱 품질이 상한.
